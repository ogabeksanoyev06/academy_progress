...
const actions = {
	async getSkillTree({commit}) {
		try {
			...
			await this._vm.$api.get('Main/Referance/SkillCourseTree').then(data => {
	...
	async getSubjectTree({commit}) {
		try {
			commit('setLoading', true);
			await this._vm.$api.get('Main/Referance/SubjectCourseTree').then(data => {
				if (!data.error && data.result) {
	...
	async getCoursesBySubjectId({commit}, payload) {
		...
		try {
			commit('setLoading', true);
			await this._vm.$api.get(`Main/Home/CourseList?subjectId=` + payload).then(data => {
				if (!data.error && data.result) {
	...
	async getAllCourses({commit}, payload) {
		...
		try {
			commit('setLoading', true);
			await this._vm.$api.get(`Main/Course/GetAll?skip=${payload.skip}&take=${payload.take}`).then(data => {
				if (!data.error && data.result) {
	...
	async getContentComments({commit}, payload) {
		...
		try {
			commit('setLoading', true);
			await this._vm.$api.get(`Main/ContentComment/GetAll?contentId=${payload}`).then(data => {
				if (!data.error && data.result) {
